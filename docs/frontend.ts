...
let state = {
    ...
    currentItem: null as { id: string, name: string } | null,
    score: 0,
    answered: 0,
    total: 10
};
...
function showScreen(screenName: 'start' | 'question' | 'answer' | 'end') {
...
elements.choiceBtns.forEach(button => {
    button.addEventListener('click', async (e) => {
        const choice = (e.target as HTMLButtonElement).dataset.choice;
        if (!choice || !state.currentItem) return;
...
elements.nextBtn.addEventListener('click', () => {
    if (state.currentItem) {
        ...
        let message = '';
        if (state.score <= 3) message = "Damn. You're reallyfix bad at this.";
